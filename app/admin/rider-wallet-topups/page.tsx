...
export default function AdminRiderWalletTopupsPage() {
  ...
  const [topups, setTopups] = useState<(RiderWalletTopup & { rider?: Profile })[]>([]);
  ...
  async function load() {
    ...
    const { data: profiles } = await supabase.from("profiles").select("*").in("id", riderIds.length ? riderIds : ["-"]);

    setTopups(
      (data || []).map((t: any) => ({
        ...t,
        rider: (profiles as Profile[] || []).find((p) => p.id === t.rider_id),
      }))
    );
  ...
  async function approve(id: string) {
    setBusyId(id);
    const res = await fetch(`/api/admin/rider-wallet-topups/${id}/approve`, { method: "POST" });
              ...
              {rejectReasonId === t.id ? (
                <div className="space-y-2">
                  <input
                    ...
                    placeholder="Reason for rejecting (shown to rider)"
                    value={rejectReason}
        ...
        <div className="space-y-2">
          {reviewed.map((t) => (
            <div key={t.id} className="card p-3 flex items-center justify-between text-sm">
              <div>
                <p className="font-medium">{t.rider?.full_name || "Rider"}</p>
